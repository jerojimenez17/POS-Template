...
async function main() {
  ...
  await prisma.$transaction(async (tx) => {
    for (const update of updates) {
      // The BusinessFeatures table is migrated away in the current schema, but
      // this script may still be run against a snapshot DB that has it. Cast to
      // any so the generated client does not block the build.
      await (tx as unknown as Record<string, unknown>).businessFeatures.update({
        where: { id: update.id },
        data: {
