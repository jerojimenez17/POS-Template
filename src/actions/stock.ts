...
const getProductsPaginatedWithRanking = async (
  ...
  try {
      ...
      Prisma.sql`
        ...
        WHERE p."businessId" = ${businessId}
          AND (
            ${codeOnly
              ? Prisma.sql`
                   similarity(LOWER(COALESCE(p.code, '')), ${normalizedSearch}) > 0
                   OR similarity(LOWER(COALESCE(p.code, '')), LOWER(${search})) > 0
                   OR similarity(LOWER(COALESCE(p.codebar, '')), ${normalizedSearch}) > 0
                   OR similarity(LOWER(COALESCE(p.codebar, '')), LOWER(${search})) > 0
                `
              : Prisma.sql`
                    LEFT(COALESCE(p.description, ''), LENGTH(${normalizedSearch})) ILIKE ${normalizedSearch}
                    OR
                    similarity(LOWER(COALESCE(p.description, '')), ${normalizedSearch}) > 0
                   OR similarity(LOWER(COALESCE(p.code, '')), ${normalizedSearch}) > 0
                   OR similarity(LOWER(COALESCE(p.code, '')), LOWER(${search})) > 0
                   OR similarity(LOWER(COALESCE(p.codebar, '')), ${normalizedSearch}) > 0
                   OR similarity(LOWER(COALESCE(p.codebar, '')), LOWER(${search})) > 0
                   OR similarity(LOWER(COALESCE(b.name, '')), ${normalizedSearch}) > 0
                   OR similarity(LOWER(COALESCE(s.name, '')), ${normalizedSearch}) > 0
                `
            }
          )
         ...
         ORDER BY
            (
              LOWER(COALESCE(p.code, '')) = ${normalizedSearch}
              OR LOWER(COALESCE(p.codebar, '')) = ${normalizedSearch}
            ) DESC,
           ${codeOnly
             ? Prisma.empty
             : Prisma.sql`
                 CASE
                   WHEN LEFT(COALESCE(p."description", ''), LENGTH(${normalizedSearch})) ILIKE ${normalizedSearch} THEN 1
                   ELSE 0
                 END DESC,
               `}
           (
            ${codeOnly
              ? Prisma.sql`
                  GREATEST(
                   COALESCE(similarity(LOWER(COALESCE(p.code, '')), ${normalizedSearch}), 0),
                   COALESCE(similarity(LOWER(COALESCE(p.code, '')), LOWER(${search})), 0)
                  ) * 2.0
                  + GREATEST(
                     COALESCE(similarity(LOWER(COALESCE(p.codebar, '')), ${normalizedSearch}), 0),
                     COALESCE(similarity(LOWER(COALESCE(p.codebar, '')), LOWER(${search})), 0)
                  ) * 1.5
                `
              : Prisma.sql`
                   COALESCE(similarity(LOWER(COALESCE(p.description, '')), ${normalizedSearch}), 0) * 3.0
                  + GREATEST(
                     COALESCE(similarity(LOWER(COALESCE(p.code, '')), ${normalizedSearch}), 0),
                     COALESCE(similarity(LOWER(COALESCE(p.code, '')), LOWER(${search})), 0)
                  ) * 2.0
                  + GREATEST(
                   ...
                   + COALESCE(similarity(LOWER(COALESCE(b.name, '')), ${normalizedSearch}), 0) * 1.0
                   + COALESCE(similarity(LOWER(COALESCE(s.name, '')), ${normalizedSearch}), 0) * 0.5
                `
            }
           ) DESC,
           ${codeOnly ? Prisma.empty : Prisma.sql`COALESCE(p."description", '') ASC,`}
           p."id" ASC
      `
    );
    ...
    const isSearchRankingFallback = Boolean(search.trim()) && !filters.codeOnly;
    const [productsResult, total] = await db.$transaction([
      db.product.findMany({
        where,
        include: { brand: { select: { id: true, name: true } }, images: { select: { id: true, url: true } } },
        orderBy: isSearchRankingFallback ? { id: "asc" } : { description: "asc" },
        ...(isSearchRankingFallback ? {} : { skip, take: pageSize }),
      }),
      db.product.count({ where }),
    ]);

    let sortedProducts = isSearchRankingFallback
      ? sortByFallbackSearchRanking(productsResult, search).slice(skip, skip + pageSize)
      : productsResult;
    if (filters.codeOnly) {
...
const sortByExactCode = <T extends { code: string | null; codebar: string | null }>(
  ...
  return products.sort((a, b) => {
    const aExact = [a.code, a.codebar].some(
      (value) => value?.trim().toLowerCase().replace(/"/g, "-") === normalizedSearch,
    ) ? 0 : 1;
    const bExact = [b.code, b.codebar].some(
      (value) => value?.trim().toLowerCase().replace(/"/g, "-") === normalizedSearch,
    ) ? 0 : 1;
...
const sortByFallbackSearchRanking = <
  T extends { id: string; code: string | null; codebar: string | null; description: string | null },
>(products: T[], search: string): T[] => {
  const normalizedSearch = search.trim().toLowerCase().replace(/"/g, "-");
  ...
  return [...products].sort((a, b) => {
    ...
    return compareDescriptionPrefix(a, b, normalizedSearch);
  });
};

const compareDescriptionPrefix = <T extends { id: string; description: string | null }>(
  a: T,
  b: T,
  ...
  const aPrefix = aDescription.startsWith(normalizedSearch) ? 0 : 1;
  const bPrefix = bDescription.startsWith(normalizedSearch) ? 0 : 1;

  if (aPrefix !== bPrefix) return aPrefix - bPrefix;

  const aNull = a.description === null ? 1 : 0;
  const bNull = b.description === null ? 1 : 0;
  if (aNull !== bNull) return aNull - bNull;

  const descriptionOrder = aDescription.localeCompare(bDescription);
  return descriptionOrder !== 0 ? descriptionOrder : a.id.localeCompare(b.id);
};
